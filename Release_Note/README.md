...
- 댓글 기능 구현 (완료)
  - 댓글 좋아요/싫어요 기능 (20241023 완료)
  - 대댓글 기능 (20241023 완료)
  - 비 Login 및 댓글이 공란인 경우 예외처리 (20241023 완료)
...
- Logo명 수정 (20241119 완료)
  - Before : Logo
  - After : POP Movie
- Title명 수정 (20241119 완료)
  - Before : React App
  - After : POP Movie

2. ModifyPage

- Input Password 확인에서 비밀번호 6자 미만 Error 체크하는 로직 추가 (20241119 완료)
- Input Email disabled 옵션 추가 (20241119 완료)
- Password 변경 기능 구현 (20241119 완료)
...
- 비Login 인 경우 영화 및 댓글에 대한 좋아요/싫어요 예외처리 (20241121 완료)

2. 유효하지 않는 페이지 예외처리

- NotFound 페이지로 이동하게끔 개발 (20241121 완료)
- CSS 적용 (진행중)
- 부트스트랩 npx 설치 후, 서브메뉴에 밑줄 생기는 현상 수정 (진행예정)
...
- NotFound 페이지로 이동하게끔 개발 (20241121 완료)
- 존재하지 않는 movie Id로 이동시에도 NotFound페이지로 이동하게끔 개발 (20241123 완료)

2. CSS 적용 (20241123 완료)

- 페이지 안내문 가운데 배치

3. 부트스트랩 npx 설치 후, 서브메뉴에 밑줄 생기는 현상 수정 (진행예정)

---

20241125(월)

1. 부트스트랩 npx 설치 후, 서브메뉴에 밑줄 생기는 현상 수정 (20241125 완료)
2. 버튼 부트스트랩 적용 완료 (20241125 완료)

---

20241127(수)

1. 패스워드 찾기 기능 제거 (20241127 완료)
2. github 배포 (진행중)
...
- 배포 (완료)
- Auth 기능이 제대로 동작하지 않고 있는 현상 (수정예정)
...
- 배포 (20241203 완료)
- Auth 기능이 제대로 동작하지 않고 있는 현상 (20241203 진행중)
...
- 탈퇴한 회원의 Comment가 존재 할 경우, 탈퇴한 회원의 닉네임을 불러오지 못하는 현상 발생
  - '탈퇴한 회원' 으로 닉네임 예외처리 (완료)

2. LandingPage

- 개발환경에서만 기능 구현이 가능한 메뉴들 예외처리 (prod환경인 경우 보이지 않게 완료)
...
- 개발환경에서만 기능 구현이 가능하여 예외처리 (prod환경인 경우 보이지 않게 완료)
